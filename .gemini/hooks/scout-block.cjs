 ...
 *
 * Blocks access to directories listed in the shipped .gemini/.ckignore baseline
 * plus an optional project-local .gemini/.ckignore override at the git root.
 ...
 *
 * Configuration:
 * - Edit .gemini/.ckignore to customize shipped baseline patterns
 * - Add <project-root>/.gemini/.ckignore to override locally without changing the baseline
...
try {
  ...
  try {
    ...
    const claudeDir = path.dirname(__dirname); // Go up from hooks/ to .gemini/
