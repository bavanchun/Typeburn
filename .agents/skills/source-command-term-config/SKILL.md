---
name: "source-command-term-config"
...
# source-command-term-config

Use this skill when the user asks to run the migrated source command `term-config`.
...
**Chezmoi root:** `/Users/vchun/.local/share/chezmoi`
...
Nếu user cung cấp path `~/.config/...`, nhắc nhở: "File đó do chezmoi quản lý — tôi sẽ edit trong source tại `/Users/vchun/.local/share/chezmoi/...` thay thế."
...
```bash
git -C /Users/vchun/.local/share/chezmoi add <source-file-path>
git -C /Users/vchun/.local/share/chezmoi commit -m "<app>: <mô tả ngắn thay đổi>"
git -C /Users/vchun/.local/share/chezmoi push
```
