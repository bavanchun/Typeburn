...
- `internal/cli/updateui` is a *second, independent* Bubble Tea program — the animated `typeburn update` block. It shares no state with `app.Model`, renders inline (no altscreen), and lives under `internal/cli` beside `notui`/`output` because it decorates a CLI command rather than being a TUI screen. It renders only; `cmd_update_run.go` owns the `Apply` goroutine and the TTY/width gate that falls back to plain text.
...
- Release binaries are **unsigned** by design (v1 scope); integrity = HTTPS + `checksums.txt` only (see `SECURITY.md`). Homebrew is **live**: `.goreleaser.yaml` has a real `homebrew_casks:` block that commits a cask into the `bavanchun/homebrew-tap-typeburn` repo, using `HOMEBREW_TAP_TOKEN` injected at the GoReleaser step in `release.yml`. Do not delete it as dead YAML. The cask is not a release asset, so the asset count stays 7. See `CONTRIBUTING.md` → *Homebrew*.
