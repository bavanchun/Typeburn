...
- `result_graph`: dual-axis Result chart — braille WPM line (left Y) + error `x` markers (right Y), X = seconds. The chart fills the width it is given: long runs downsample into equal buckets, short runs stretch across the whole cell budget with samples spread edge to edge. Stretching gives the connecting segment more pixels rather than inventing intermediate samples, so no WPM value the test did not measure is ever plotted. The right axis is omitted entirely when no second carried an error — that decision is made *before* the cell budget, since it frees columns. Geometry lives in `result_graph_geometry.go` (`graphGeometryFor`), braille cell packing in `result_graph_braille.go`. History keeps its own inline bar sparkline (`sparklineInline`).
- `result_layout`: the Result screen's single width policy (`layoutFor`). Returns `{PanelW, InnerW, LeftPad}`; content is capped at `resultMaxContentW` and the panel is centred in the remaining space. Every section sizes itself from this — four independent width policies is what previously let a 192-column panel hold about 35 columns of content.
