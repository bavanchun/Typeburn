...
- `result_graph`: dual-axis Result chart — braille WPM line (left Y) + error `x` markers (right Y), X = seconds; long runs downsample into equal buckets. History keeps its own inline bar sparkline (`sparklineInline`).
...
- **Result reveal** (screen_result_reveal.go, screen_result_hero.go): WPM count-up in a fixed-width digit slot, graph draw-in (a `visible` count on `RenderResultGraph`), staggered acc block + stat cards.
