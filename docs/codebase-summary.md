...
- `replace_unix.go` / `replace_windows.go`: atomic same-dir rename; the Windows path moves the running exe aside and rolls back if the second rename fails, so the target is never left missing. There is deliberately **no** startup crash-recovery pass — a crash between the two renames leaves the target absent, and recovery that only runs when typeburn is launched *from* the target could never execute in that state.
...
- `ResultModel`: two-big-number hero (wpm big-digits + acc), dual-axis WPM/errors line graph, 2-column stats grid, most-missed heatmap, footer navigation.
...
- `result_graph`: dual-axis Result chart — braille WPM line (left Y) + error `x` markers (right Y), X = seconds. The chart fills the width it is given: long runs downsample into equal buckets, short runs stretch across the whole cell budget with samples spread edge to edge. Stretching synthesizes no samples — it widens the segment already drawn between two measured seconds. That segment is linearly interpolated (as it was before stretching), so the vertices are measurements and the line between them is not. The right axis is omitted entirely when no second carried an error — that decision is made *before* the cell budget, since it frees columns. Geometry lives in `result_graph_geometry.go` (`graphGeometryFor`), braille cell packing in `result_graph_braille.go`. History keeps its own inline bar sparkline (`sparklineInline`).
- `result_layout`: the Result screen's single width policy (`layoutFor`). Returns `{PanelW, InnerW}`; content is capped at `resultMaxContentW`. It deliberately does *not* centre — `ResultModel.View` already places the frame with `lipgloss.Place`, and a layout that padded as well centred the panel twice and pushed it right. The panel, graph, and key heatmap size themselves from it; the hero and stats grid need no width because the cap already guarantees their content fits. Four independent width policies is what previously let a 192-column panel hold about 35 columns of content.
...
- **Result reveal** (screen_result_reveal.go, screen_result_hero.go): WPM count-up in a fixed-width digit slot, graph draw-in (a `visible` count on `RenderResultGraph`), staggered acc block + stat cards. Every mid-reveal frame keeps the settled frame's line count and per-line width, and the fully-revealed frame is byte-identical to the static render; both are enforced by tests, including for stretched charts.
...
**Files (by screen):** screen_home.go, screen_home_view.go, screen_home_test.go; screen_typing.go, screen_typing_view.go, screen_typing_actions.go, screen_typing_test.go; screen_result.go, screen_result_view.go, screen_result_hero.go, screen_result_test.go; screen_settings.go, screen_settings_view.go, screen_settings_test.go; screen_history.go, screen_history_view.go, screen_history_test.go; screen_code_paste.go, screen_code_paste_view.go, screen_code_paste_test.go. Plus: footer.go, timer.go, stat_card.go, result_graph.go, result_graph_axes.go, result_graph_geometry.go, result_graph_braille.go, result_layout.go, word_stream_renderer.go, ascii_big_digits.go, ascii_logo.go, degraded_notice.go, selectable_list.go, settings_rows.go, width_tier.go, history_table.go, result_render_helpers.go, typing_log_helpers.go, test_helpers_test.go, phase09_polish_test.go, ui.go.
